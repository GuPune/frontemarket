<template>
        <div class="card shopping-cart">
                  <h2 class="shoping-cart-title bg-order"><span>{{placeholder_deli}}</span></h2>
...
import { CHOOSE_DELIVERY,CHECK_DELIVERY,DELIVERY_DATA } from "../store/actions.type.js";
  export default {
   ...
   computed: {
        ...
        if(this.language == 'en'){
         this.placeholder_deli = 'Shipping method';


        }
        if(this.language == 'ch'){
          this.placeholder_deli = '邮寄方式';


        }
        if(this.language == 'th' || this.language == null){
          this.placeholder_deli = 'วิธีการจัดส่ง';
